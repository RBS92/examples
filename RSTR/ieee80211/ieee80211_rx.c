...
#endif
#include "ieee80211.h"
#include "../rtl8187/r8187.h"
#define RTL8187_MODULE_NAME "ieee80211"
#define DMESG(x,a...) printk(KERN_INFO RTL8187_MODULE_NAME ": " x "\n", ## a)

extern void mg_reception(struct ieee80211_device *ieee, struct mg_net_data *rxb, struct ieee80211_hdr *hdr);

static inline void ieee80211_monitor_rx(struct ieee80211_device *ieee,
					struct sk_buff *skb,
					struct ieee80211_rx_stats *rx_stats)
{
	struct r8180_priv *priv = ieee80211_priv(ieee->dev);
	struct mg_net_data *rxb;
	...
	skb_pull(skb, 18);
	rxb = (struct mg_net_data *)skb->data;
	
	/*
	if(priv->mg_net_data->nconn < 3)
	{
		if(!memcmp(hdr->addr3,priv->mg_net_data->preamble, ETH_ALEN) && (priv->mg_node_data->val != rxb->val) )
		{
			//priv->mg_node_data->val[3] = priv->mg_node_data->val[2];
			//priv->mg_node_data->val[2] = priv->mg_node_data->val[1];
			//priv->mg_node_data->val[1] = priv->mg_node_data->val[0];
			priv->mg_node_data->val = rxb->val;
			memcpy(priv->mg_node_data->last_src, hdr->addr2, ETH_ALEN);
			mg_reception(ieee,rxb,hdr);					
		}
	}
	else*/
	//{
	
	if(!memcmp(hdr->addr3,priv->mg_net_data->preamble, ETH_ALEN) && (memcmp(&priv->mg_node_data->timestamp, &rxb->timestamp,sizeof(unsigned long)) || rxb->timestamp == 0) )// && (priv->mg_node_data->val != rxb->val))// && memcmp(priv->mg_node_data->last_src, hdr->addr2, ETH_ALEN)) )
	{
		priv->mg_node_data->timestamp = rxb->timestamp;
		//priv->mg_node_data->val[3] = priv->mg_node_data->val[2];
		//priv->mg_node_data->val[2] = priv->mg_node_data->val[1];
		//priv->mg_node_data->val[1] = priv->mg_node_data->val[0];
		//priv->mg_node_data->val = rxb->val;
...
#ifdef NOT_YET
	//R if (hostap_rx_frame_wds(ieee, hdr, fc, &wds))
	//	goto rx_dropped;
	if (wds) {
		skb->dev = dev = wds;
...
#ifdef NOT_YET
	/*if (sta)
		hostap_handle_sta_release(sta);*/ //R FUCk ALL THE HOSTAPP SHIT.
#endif
       	return 1;
