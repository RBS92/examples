...
#define RTL8187_MODULE_NAME "MicroGrid"
...
struct mg_node_data
{
	bool islanded; 		//state of node and set to islanded (MAKE ENUM!)

	bool new_val;	        // TRUE if new values are received since last broadcast.
	u8 npend;
	u8 *pend_table;
	u16 voltbuffer;
	u16 freqbuffer;
	...
	//int not_dub;
	//u8 Conn_table[20][ETH_ALEN];     // Connection table
	//u8 last_src[ETH_ALEN];
};

struct mg_net_data
{
	u8 dest[ETH_ALEN], src[ETH_ALEN];
	u8 preamble[ETH_ALEN];// = {0xDE,0xAD,0xBE,0xEF};
	u8 token_pass;     // Flag - this is only a token pass, dont save data upon reception
	//int sender_id;       // Addressor
	//int receiver_id;     // Adressee
	//u8 pckt;	//A random byte to kill mesh re-transmissions
	u8 test;
	//u32 val;
	u8 volt;            // Voltage of sending node
	u8 freq;            // Freq of sending node
	u8 pow;             // Power of sending node
	u8 nconn;
	u8 Conn_table[ETH_ALEN*20];  // Connection table
	unsigned long timestamp;
};

/*struct timer_list { unsigned long expires;      //R
...
typedef struct r8180_priv
{
	...
	struct net_device *dev;
    	...
    	struct mg_node_data *mg_node_data;  //R
    	struct mg_net_data *mg_net_data; //R
	struct work_struct mg_broadcast_work; //R
	struct workqueue_struct *mg_broadcast_wq; //R
