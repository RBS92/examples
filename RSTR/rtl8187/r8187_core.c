...
short rtl8180_init(struct net_device *dev)
{
	...
	priv->ps_sleep_finish=0;
	priv->ieee80211->iw_mode = IW_MODE_MONITOR; //R
	...
	priv->ieee80211->mode = IEEE_G|IEEE_B; //R
	...
	priv->ieee80211->iw_mode = IW_MODE_MONITOR;	//R
	priv->ieee80211->softmac_features  = IEEE_SOFTMAC_SCAN | 	//R
	...
	switch (priv->rf_chip) {
		...
		DMESGW("**PLEASE** REPORT SUCCESS/INSUCCESS TO GR505");
...
/****************************************************************************
     ---------------------------- CONTROL_STUFF---------------------------
*****************************************************************************/
//#define MGMAXMISS 10	//number of reception misses until removal of prior node

#if 1
//table_lookup looks in the table k, for the integer m.
int table_lookup(u8 *address, u8 *table)
{
    int ret = -1;
    int i;
    for (i=0;i<20;i++)
    {
        if(!memcmp(address,(table + ETH_ALEN*i),ETH_ALEN))
		ret = i;
    }
    return ret;
}
...
void mg_reception(struct ieee80211_device *ieee, struct mg_net_data *rxb, struct ieee80211_hdr *hdr)
{
	struct net_device *dev = ieee->dev;
	struct r8180_priv *priv = ieee80211_priv(dev);

	u8 probe[ETH_ALEN] = {0x00,0x00,0x00,0x00,0x00,0x00};
	unsigned long flags;
	...
	//udelay(1000);
	//skb_pull(skb, 18);
//if(rxb->test > priv->mg_net_data->test || (rxb->test == 0) )
//{
	//DMESG("Reception Node: %i",*(dev->dev_addr+5));
	//DMESG("RXID %lx",rxb->val);
//if(priv->mg_net_data->test != rxb->test || rxb->test == 0) // IF NOT A DUPLICATE only true for first new packet values
//{
	priv->mg_net_data->test = rxb->test+1;
	// Check whether it is a probe: (DESTINATION == 0x00 0x00 0x00 0x00 0x00 0x00)

	if(!(memcmp(hdr->addr1, probe, ETH_ALEN)))
	{
		//DMESG("Received probe");
		//rtl8180_rtx_disable(dev);
		if(!(table_lookup(hdr->addr2,priv->mg_net_data->Conn_table) + 1))
		{
			//DMESG("Not in ring");
			if (priv->mg_node_data->npend<3)
			{
				memcpy((priv->mg_node_data->pend_table + ETH_ALEN*(priv->mg_node_data->npend)), hdr->addr2, ETH_ALEN);
				priv->mg_node_data->npend++;
				if(priv->mg_node_data->islanded)// && priv->mg_node_data->probed_fix)
				{
					del_timer(priv->timer);
					//priv->mg_node_data->probed_fix = 0;
					queue_work(priv->mg_broadcast_wq, &priv->mg_broadcast_work);
				}
			}
			else
			{
				//DMESG("PENDING QUEUE FULL");
	...
	{
		//priv->mg_node_data->new_val = true;   //simulating values

		/*if(priv->mg_node_data->islanded && (table_lookup(dev->dev_addr, rxb->Conn_table) + 1) ) //if expected to be part of the network
		{
			// Set as part of the network
			del_timer(priv->timer);
			mod_timer(priv->timer, jiffies + HZ*60); //A SAFETY IF THE NETWORK DISCONNECTS THIS NODE BEFoRE IT RECEIVES A TOKEN
			priv->mg_net_data->nconn = rxb->nconn;
			memcpy(priv->mg_net_data->Conn_table, rxb->Conn_table, ETH_ALEN*rxb->nconn);
			priv->mg_node_data->islanded = false;
		}*/

		//priv->mg_node_data->not_dub = 0;
		// If not a token pass. We must handle the data
		if(!rxb->token_pass)// && (priv->mg_node_data->val != rxb->val))
		{
			//priv->mg_node_data->not_dub = 1;
			//priv->mg_node_data->val = rxb->val;
			//DMESG("Received data");
			//COPY DATA AND DO AVERAGE
            priv->mg_node_data->voltbuffer  =  rxb->volt + priv->mg_node_data->voltbuffer;
            priv->mg_node_data->freqbuffer  =  rxb->freq + priv->mg_node_data->freqbuffer;
            priv->mg_node_data->powbuffer   =  rxb->pow + priv->mg_node_data->powbuffer;
            //DMESG("Jiffies %lu",&jiffies);
			//DMESG("volt: %i \n freq: %i \n pow: %i", rxb->volt, rxb->freq, rxb->pow);
		}

		//DMESG("SRC: %i",*(hdr->addr2+5));
		//DMESG("DST: %i",*(hdr->addr1+5));
		// Check if token passed to this node
		if(!memcmp(hdr->addr1, dev->dev_addr, ETH_ALEN))
		{
			//if (priv->mg_node_data->not_dub)
			DMESG("Node %i: Counter %i",*(dev->dev_addr+5),rxb->test);
			//rtl8180_rtx_disable(dev);
			//COPY MAC DATA (CONNECTION TABLE and priv->mg_node_data->nconn)
			del_timer(priv->timer);
			//DMESG("Connected: %i",rxb->nconn);
			priv->mg_net_data->nconn = rxb->nconn;
			memcpy(priv->mg_net_data->Conn_table, rxb->Conn_table, ETH_ALEN*rxb->nconn);
			//DMESG("Conns: 1: %i: 2: %i 3: %i: 4: %i",*(priv->mg_net_data->Conn_table + 5),*(priv->mg_net_data->Conn_table+ETH_ALEN+5),*(priv->mg_net_data->Conn_table+ETH_ALEN*2+5),*(priv->mg_net_data->Conn_table+ETH_ALEN*3+5) );
			//DMESG("Conns: 5: %i: 6: %i 7: %i: 8: %i",*(priv->mg_net_data->Conn_table+ETH_ALEN*4 + 5),*(priv->mg_net_data->Conn_table+ETH_ALEN*5+5),*(priv->mg_net_data->Conn_table+ETH_ALEN*6+5),*(priv->mg_net_data->Conn_table+ETH_ALEN*7+5) );

			//DMESG("Token passed to this node");
			priv->mg_node_data->missed = 0;
			priv->mg_node_data->islanded = false;

            averagevolt = (priv->mg_net_data->volt + priv->mg_node_data->voltbuffer)/(priv->mg_net_data->nconn);
            averagefreq = (priv->mg_net_data->freq + priv->mg_node_data->freqbuffer)/(priv->mg_net_data->nconn);
            averagepow = (priv->mg_net_data->pow + priv->mg_node_data->powbuffer)/(priv->mg_net_data->nconn);
            DMESG("AVERAGE VOLT: %i\nAVERAGE FREQ: %i\nAVERAGE POW: %i", averagevolt, averagefreq, averagepow);

            priv->mg_node_data->voltbuffer = 0;
            priv->mg_node_data->freqbuffer = 0;
            priv->mg_node_data->powbuffer = 0;

			//queue_work(priv->mg_broadcast_wq, &priv->mg_broadcast_work);
			//priv->mg_node_data->retrans = 0; //variable for testing
			mod_timer(priv->timer, jiffies + HZ/100); //reschedule with 10 MS delay
		}
	}
//}
//else
...
unsigned int mg_rescheduletime(unsigned int nconn)
{
	unsigned int random;
	unsigned int mg_time;
	get_random_bytes(&random,1);

	mg_time = HZ;// + mg_time;// + mg_random/100;	//A second is the maximum time for a cycle in the MG_token ring
	// make the time function
	return mg_time;
}


void mg_broadcast_work(struct work_struct *work)
{
	struct net_device *dev;
	...
	unsigned long flags;
	int i;
	u8* dest, probe[ETH_ALEN] = {0x00,0x00,0x00,0x00,0x00,0x00};

	priv = container_of(work, struct r8180_priv, mg_broadcast_work);
	dev = priv->dev;
	ieee = netdev_priv(dev);

	//down(&priv->wx_sem);
	spin_lock_irqsave(&ieee->lock, flags);
	//priv->mg_node_data->nQueue = 1;

	//priv->mg_node_data->retrans = 1; //variable for testing
#if 1
    	///simulate new nodedata///

	//if (priv->mg_node_data->new_val)
    	//{
	priv->mg_net_data->token_pass = false;
    	priv->mg_net_data->volt = 0x11;
    	priv->mg_net_data->freq = 0xBB;
    	priv->mg_net_data->pow = 0xAA;

	//}
	//else
	//	priv->mg_net_data->token_pass = true;

	///end of simulate new nodedata///


	//remove last node from connectiontable if too many misses
	if(priv->mg_node_data->missed > 10)//MGMAXMISS)
	{
		//DMESG("Faulty node removed");
		priv->mg_node_data->missed = 0;
		//Remove unresponsive node from network (connection table)
		u8 this = table_lookup(dev->dev_addr,priv->mg_net_data->Conn_table);

		if(!this && this > -1)
			for (i = this; i < (priv->mg_net_data->nconn-1); i++)
				memcpy(priv->mg_net_data->Conn_table + ETH_ALEN*(i-1), priv->mg_net_data->Conn_table + ETH_ALEN*i, ETH_ALEN);

		priv->mg_net_data->nconn--;
		if(priv->mg_net_data->nconn < 2)
		{
			priv->mg_node_data->islanded = true;
			//priv->mg_node_data->probed_fix = 1;
		}
...
start:
	//GET THE DESTINATION!
	if(priv->mg_node_data->npend)	//check for pending nodes
	{
		//DMESG("pending node");
		if (!(table_lookup(priv->mg_node_data->pend_table, priv->mg_net_data->Conn_table)+1))
		{
		//node has not been added - add it and remove from pending
			//DMESG("pending is not in network");
			//Save as destination
			memcpy(priv->mg_net_data->dest, priv->mg_node_data->pend_table, ETH_ALEN);

			//ADD to connection table (depending on islanded or connected)
			if(priv->mg_node_data->islanded)
			{
				//DMESG("islanded: ADDING to connection table");
				//ADD both this and pending to the connection table
				memcpy(priv->mg_net_data->Conn_table, dev->dev_addr, ETH_ALEN); //THIS NODE
				memcpy(priv->mg_net_data->Conn_table + ETH_ALEN, priv->mg_net_data->dest, ETH_ALEN); //PENDING NODE
				priv->mg_net_data->nconn = 2;
				priv->mg_node_data->islanded = false;
				//DMESG("Conn 1: %i", *(priv->mg_net_data->Conn_table +5));
				//DMESG("Conn 2: %i", *(priv->mg_net_data->Conn_table +5 + ETH_ALEN));
			}
			else
			{
				u8 this = table_lookup(dev->dev_addr,priv->mg_net_data->Conn_table);

				for (i = (priv->mg_net_data->nconn -1); i > this;i--)
					memcpy(priv->mg_net_data->Conn_table + ETH_ALEN*(i+1), priv->mg_net_data->Conn_table + ETH_ALEN*i, ETH_ALEN);
				memcpy(priv->mg_net_data->Conn_table + ETH_ALEN*(this+1), priv->mg_net_data->dest, ETH_ALEN);


				priv->mg_net_data->nconn++;
			}

			for(i = 0; i < (priv->mg_node_data->npend-1); i++)
				memcpy(priv->mg_node_data->pend_table + ETH_ALEN*i, priv->mg_node_data->pend_table + ETH_ALEN*(i+1), ETH_ALEN);
			priv->mg_node_data->npend--;
		}
		else
		{
	...
	else if (priv->mg_node_data->islanded)
	{
		//DMESG("Islanded node sends probe!");
		memcpy(priv->mg_net_data->dest, probe, ETH_ALEN);
	}
	else if (table_lookup(dev->dev_addr, priv->mg_net_data->Conn_table)+1)	//NO PENDING AND CONNECTED -> DEVADDRESS is the next in the conn_table
	{
		u8 dnumb = (table_lookup(dev->dev_addr, priv->mg_net_data->Conn_table) + 1);
		//DMESG("part of network");
		if(dnumb != priv->mg_net_data->nconn)
		{
			memcpy(priv->mg_net_data->dest, (priv->mg_net_data->Conn_table + dnumb*ETH_ALEN), ETH_ALEN);
		}
		else
		{
			memcpy(priv->mg_net_data->dest, priv->mg_net_data->Conn_table, ETH_ALEN);
		}
	}
	else //not part of the network anymore!
	{
		priv->mg_node_data->islanded = 1;
		memcpy(priv->mg_net_data->dest, probe, ETH_ALEN);
	}
	...
	if(priv->mg_node_data->islanded)
		priv->mg_net_data->timestamp = 0;
	else
		priv->mg_net_data->timestamp = jiffies;
#endif
#if 1


    	//allocate skb
    	int user_data_len = sizeof(struct mg_net_data);
    	skb = dev_alloc_skb(user_data_len);

	if (!skb)
        	DMESG("SKB ALLOCATION ERROR");
    	//make space for header
    	//skb_reserve(skb, IEEE80211_3ADDR_LEN);	//TBD THIS IS AN ERROR - INVESTIGATE

    	//Assign data
   	unsigned char *data = skb_put(skb, user_data_len);
   	int err = 0;
	skb->csum = csum_and_copy_from_user(priv->mg_net_data, data,
					    user_data_len, 0, &err);
	if (err)
		DMESG("SKB DATA ERROR");
#endif
	...
	TXB FOR SOFTMAC_MGMT_XMIT
    struct ieee80211_txb *txb;

    txb->nr_frags = 0;
    txb->reserved = 1;
	txb->frag_size = skb->len;
	txb->payload_size = skb->len;
	txb->fragments[0] = skb;
#endif

#if 1
	//Making sure that the new tx-buffer will hold! (paging error fix)
	ieee80211_txb_free(ieee->alloc_txb[ieee->tx_skb_index], MAX_NUM_FRAGS);
	ieee->alloc_txb[ieee->tx_skb_index] = ieee80211_alloc_txb(MAX_NUM_FRAGS, MAX_SIZE_TX_SKB, GFP_KERNEL);

	//prepare a new random u32
	//u32 last_val = priv->mg_node_data->val;
	//do {
	//get_random_bytes(&priv->mg_node_data->val,sizeof(priv->mg_node_data->val));
	//}  while(last_val == priv->mg_node_data->val);
	//if(priv->mg_node_data->islanded)
	//	priv->mg_net_data->timestamp = 0;
	//else
	//	priv->mg_net_data->timestamp = jiffies;
	//DMESG("Broadcast node: %i",*(dev->dev_addr+5));
	//DMESG("DST: %i\n",*(priv->mg_net_data->dest+5));
	//up(&priv->wx_sem);

	//rtl8180_rx_enable(dev);

	spin_unlock_irqrestore(&ieee->lock, flags);
	//priv->ieee80211->softmac_stats.tx_probe_rq++;
    	ieee80211_xmit(skb, dev);


	if( priv->mg_node_data->islanded )
		mod_timer(priv->timer, jiffies + HZ); //reschedule with 1 sec delay
	else
		mod_timer(priv->timer, jiffies + HZ*priv->mg_net_data->nconn*2*13/1000 ); //reschedule
#endif
}

void mg_broadcast_timer(struct net_device *dev)
{
    struct r8180_priv *priv = ieee80211_priv(dev);

	priv->mg_net_data->token_pass = true;
	//count the times prior node has missed passing the token
	if(!(priv->mg_node_data->islanded))
	{
		//priv->mg_node_data->probed_fix = 1;
		priv->mg_node_data->missed++;
		/*if(priv->mg_node_data->retrans)
		{
			//DMESG("Retransmission");
		}*/
	}
	queue_work(priv->mg_broadcast_wq, &priv->mg_broadcast_work);	//This places our work in the workqueue we've deffined (in the usbprobe)
}

void mg_init(struct net_device *dev)
{
	int i;
	struct r8180_priv *priv = ieee80211_priv(dev);

	priv->timer = kmalloc(sizeof(struct timer_list), GFP_KERNEL);
    	priv->timer->function = mg_broadcast_timer;
    	priv->timer->data = dev;

	priv->mg_node_data = kmalloc(sizeof(struct mg_node_data), GFP_KERNEL);
	priv->mg_net_data = kmalloc(sizeof(struct mg_net_data), GFP_KERNEL);

	//priv->mg_node_data->nQueue = 0;

   	memcpy(priv->mg_net_data->src, dev->dev_addr,ETH_ALEN);	//Copy src MAC
	priv->mg_node_data->islanded = true;
	priv->mg_node_data->npend = 0;
	priv->mg_net_data->nconn = 0;
	priv->mg_net_data->test = 0;
	priv->mg_node_data->missed = 0;
	memcpy(priv->mg_net_data->preamble, (u8[]) {0xDE, 0xAD,0x13,0x37,0xBE,0xEF}, ETH_ALEN);
	priv->mg_node_data->pend_table = kmalloc(4*ETH_ALEN,GFP_KERNEL);
	//priv->mg_net_data->Conn_table = kmalloc(20*ETH_ALEN,GFP_KERNEL);
	for (i=0;i<20;i++)
		memcpy(priv->mg_net_data->Conn_table + ETH_ALEN*i, (u8[]) {0x00, 0x00,0x00,0x00,0x00,0x00}, ETH_ALEN);
	priv->mg_broadcast_wq = create_workqueue("mg_broadcast");	// This is init of the workqueue.

	INIT_WORK(&priv->mg_broadcast_work,(void*) mg_broadcast_work);	// THis is the init work.

	rtl8180_open(dev);

	init_timer(priv->timer);
    	setup_timer(priv->timer, mg_broadcast_timer, dev);
	if(mod_timer(priv->timer, jiffies + 2*HZ))
		DMESG("MOD_TIMER ERROR");
	//R SCHEDULE A TIMER TO BROADCAST 	mg_broadcast(dev); //RTBD

	DMESG("MG init completed\n");
}
#endif

//DECLARE_WORK(mg_broadcast_work,(void*) mg_broadcast_handler, dev);
...
static int __init rtl8187_usb_module_init(void)
{
    ...
    printk(KERN_INFO "\nLinux kernel driver for MicroGrid network\n");
